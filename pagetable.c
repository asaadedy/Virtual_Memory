...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
        ...
        if(victim->frame & PG_DIRTY) {
            evict_dirty_count++;
       	    victim->swap_off = swap_pageout(victim->frame >> PAGE_SHIFT, victim->swap_off);

		    if (victim->swap_off == INVALID_SWAP) {
			    perror("allocate_frame: could not evit a frame from memory and place it in swap file");
                exit(-1);
		    }else {
...
char *find_physpage(addr_t vaddr, char type) {
    ...
    uintptr_t pde;
    if(!(dir_entry->pde & PG_VALID)){
        pgdir_entry_t new = (init_second_level());
        *dir_entry = new;
    }
    pde = (pgdir[idx].pde & PAGE_MASK);
	// Use vaddr to get index into 2nd-level page table and initialize 'p'
   	unsigned idx2 = PGTBL_INDEX(vaddr);
    
    p = &((pgtbl_entry_t *) pde)[idx2];

	// Check if p is valid or not, on swap or not, and handle appropriately
	if(!(p->frame & PG_VALID)) {
       	int frame = allocate_frame(p);    // Frame is invalid, so allocate
        coremap[frame].virt_addr = vaddr;

       	// Check if frame has not been allocated yet
       	if(!(p->frame & PG_ONSWAP)) {
           	init_frame(frame, vaddr);
            p->frame = (frame << PAGE_SHIFT);
            // Mark new frame dirty, according to piazza post
